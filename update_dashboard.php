...
$search = '/\/\/ Calculer Réel \(Revenus du mois\).*?\[\s*\'grid\'\s*=>\s*\[\s*\'color\'\s*=>\s*\'#1e293b\'\s*\]\,\s*\'ticks\'\s*=>\s*\[\s*\'color\'\s*=>\s*\'#a0aec0\'\s*\]\s*\]\s*\]\s*\]\);/s';
...
if ($newContent === null || $newContent === $content) {
    echo "Replacement failed or nothing changed.\n";
} else {
    file_put_contents($file, $newContent);
    echo "Dashboard Siege updated successfully.\n";
}
